...
function BannerCarousel() {
  ...
  const defaultBanners = [
    { 
      _id: "b1", 
      title: "India's Smart Scrap Management Platform - ScrapVex", 
      imageUrl: "/05_Hero_Banner_01.png", 
      mobileImageUrl: "/05_Hero_Banner_01.png", 
      link: "/book", 
      isActive: true 
    }
  ];

  // On Mobile, always prioritize the single official responsive banner
  const activeBanners = isMobile ? defaultBanners : (ads.length > 0 ? ads : defaultBanners);
    ...
    <div style={container} className="reveal active">
      ...
      >
        {activeBanners.map((ad, index) => {
          ...
          return (
            <div
              ...
              style={{
                ...slide,
                opacity: (activeBanners.length === 1 || index === current) ? 1 : 0,
                transform: `scale(${(activeBanners.length === 1 || index === current) ? 1 : 0.97})`,
                zIndex: (activeBanners.length === 1 || index === current) ? 1 : 0,
              }}
            >
              <a href={ad.link || "/book"} style={{ display: "block", width: "100%", height: "100%" }}>
                <img 
                  src={bannerImg} 
                  alt={ad.title} 
                  style={{
        ...
        {activeBanners.length > 1 && (
          <>
            <button style={leftBtn} onClick={() => setCurrent(current === 0 ? activeBanners.length - 1 : current - 1)} aria-label="Previous">
              <FaChevronLeft size={isMobile ? 12 : 16} />
            </button>
            <button style={rightBtn} onClick={() => setCurrent(current === activeBanners.length - 1 ? 0 : current + 1)} aria-label="Next">
